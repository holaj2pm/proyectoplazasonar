...
if (
    isset($_POST['selected_employee']) &&
    isset($_POST['selected_services'])
) {
    ?>
    <!-- CALENDAR STYLE -->
    ...
    <style type="text/css">
        .calendar_tab {
            ...
            width: 100%;
            position: relative;
            box-shadow: rgba(60, 66, 87, 0.04) 0px 0px 5px 0px,
                rgba(0, 0, 0, 0.04) 0px 0px 10px 0px;
            overflow: hidden;
            border-radius: 4px;
        }
    ...
    <div class="calendar_slots" style="min-width: 600px;">
        ...
        <div class="appointments_days">
            ...
            for ($i = 0; $i < 10; $i++) {
                ...
                echo "<br>";
                echo date('d', strtotime($appointment_date)) . " " .
                    date('M', strtotime($appointment_date));
                echo "</div>";
            }
            ?>
        </div>
        ...
        <div class='available_booking_hours'>
            ...
            foreach ($desired_services as $service) {
                ...
                $stmtServices = $con->prepare("select service_duration from 
                ...
                while ($start >= $open_time && $result <= $close_time) {
                    ...
                    if ($stmt_emp->rowCount() != 0) {
                        ...
                        if ($stmt->rowCount() != 0) {
                            ...
                            ?>
                            <input type="radio" id="<?php echo $appointment_date .
                                " " . $start; ?>" name="desired_date_time"
                                value="<?php echo $appointment_date . " " . $start . " " . $result; ?>">
                            <label class="available_booking_hour" for="<?php
                            echo $appointment_date . " " . $start; ?>">
                                <?php echo $start; ?>
                            </label>
                            <?php
                        }
